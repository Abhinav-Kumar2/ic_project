...
int clue_found[9] = {0}; 
char inventory[10][500]; //10 or so clues
...
void display_inventory() {
    ...
    if (inventory_count == 0) {
        ...
        for (int i = 0; i < inventory_count; i++) {
            printf("%s\n", inventory[i]); // Showing the entire list of the inventory
        }
    }
}
...
void examine_clue(int clue_number) {
    ...
    FILE *file = fopen("b24cm1018_b24cm1002_b24cm1053_b24cs1056_clues.txt", "r");
    ...
    while(1){
        fgets(clue_line,1000,file);
        char a[2];
        char tobecomp[2] = {clue_line[0],'\0'};
        sprintf(a,"%d",clue_number);
        if(strcmp(tobecomp,a)==0){
            printf("%s",clue_line);
...
void discover_secret_location() {
    ...
    printf("The Secret Code is 'xyz'. If you know you know.");
}
