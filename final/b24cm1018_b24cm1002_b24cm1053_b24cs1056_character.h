...
void initializeChar(Character* c, int health){
    c->mhp=health; //modifications can be made to vary the maximum hp from character to character
    c->shieldon=0;
    c->isowned[0]=1;
    c->chp=c->mhp;
    c->sanity=80;
...
void initializeOpp(Character* c) {
    Weapon weaps[NO_OF_WEAPS] = {Fist, Hammer, Knife, Pistol, Katana, SMG};
    int randomWeaponIndex = rand() % (NO_OF_WEAPS-1); // randomize weapon choice
    c->mweap = weaps[randomWeaponIndex]; 
    c->isowned[randomWeaponIndex] = 1; // random weapon as owned
    printf("%s is equipped with %s.\n", c->mname, weaps[randomWeaponIndex].mname);
}
...
void equip(Character* charac){
    ...
    while(1){
        ...
        if(choice<60)
        {
            if((choice>=0)&&charac->isowned[choice]==1){break;}
